...
final class UserManager
{
    ...
    public function __construct(UserProviderInterface|array|null $provider = null)
    {
        if ($provider instanceof UserProviderInterface) {
            ...
            PrefabRuntime::recordResolution('users', 'user_provider', 'module-local', ['provider' => $provider::class]);
    ...
    public function prefabConfigure(): void
    {
        if (!$this->provider) {
            ...
            if ($provider['value'] instanceof UserProviderInterface) {
                ...
                PrefabRuntime::recordResolution('users', 'user_provider', $provider['source'], ['provider' => $this->provider::class]);
            }
        }
        ...
        if (!$this->provider) {
            ...
            if ($database) {
                ...
                $this->provider = new PdoUserProvider(
                    $database,
                    $userMap,
                    $factory['value'] instanceof UserFactoryInterface ? $factory['value'] : null,
                );
                ...
                PrefabRuntime::recordResolution('users', 'user_provider', 'database-provider', ['provider' => PdoUserProvider::class]);
            }
        }
        ...
        if ($this->database) {
            PrefabRuntime::provide('database', $this->database, 'prefab-users', priority: -10, meta: [
                'role' => 'users-database',
        ...
        if (!$this->actorProvider) {
            ...
            if ($actor) {
                ...
                PrefabRuntime::recordResolution('users', 'actor_provider', 'prefab-capability', ['provider' => $actor['provider']]);
            }
        }
    }
    ...
    /** Show the Users execution call tree. */
    public function explain(): void
    {
        PrefabRuntime::explain('users');
    }

    /** Structured configuration-resolution data for tools/tests. */
    public function explainData(): array
    {
        return PrefabRuntime::explainData('users');
    }
    ...
    {
        return PrefabRuntime::traceCall('users', 'find', ['user_id' => $id], fn () => $this->provider()->find($id));
    }
    ...
    {
        return PrefabRuntime::traceCall('users', 'all', ['limit' => $limit, 'offset' => $offset], function () use ($limit, $offset): array {
            $users = $this->provider()->all($limit, $offset);
            PrefabRuntime::traceStep('users.returned', ['users' => count($users)]);
            return $users;
        });
    }
    ...
    {
        return PrefabRuntime::traceCall('users', 'create', ['fields' => array_keys($data)], function () use ($data, $context): OperationResult {
            $user = $this->provider()->create($data);
            PrefabRuntime::traceStep('users.created', ['user_id' => $user->id]);
            return $this->result(
                $user,
                $this->logPayload(
                    'user.created',
    ...
    {
        return PrefabRuntime::traceCall('users', 'update', ['user_id' => $id, 'fields' => array_keys($data)], function () use ($id, $data, $context): OperationResult {
            $before = $this->provider()->find($id);
            $user = $this->provider()->update($id, $data);
            PrefabRuntime::traceStep('users.updated', ['user_id' => $user->id]);
            return $this->result(
                $user,
                $this->logPayload(
                    'user.updated',
    ...
    {
        return PrefabRuntime::traceCall('users', 'delete', ['user_id' => $id], function () use ($id, $context): OperationResult {
            $before = $this->provider()->find($id);
            $deleted = $this->provider()->delete($id);
            $name = $before?->name ?? (string) $id;
            PrefabRuntime::traceStep('users.deleted', ['user_id' => $id, 'deleted' => $deleted]);
            return $this->result(
                $deleted,
                $this->logPayload(
                    'user.deleted',
    ...
    private function resolveDatabase(): array
    {
        ...
        if ($entry && $capability) {
            return [$capability, 'prefab-capability', [
    ...
    private function provider(): UserProviderInterface
    {
        ...
        if (!$this->provider) {
            ...
            $this->provider = new PdoUserProvider($this->database, $map);

            PrefabRuntime::recordResolution('users', 'database', 'automatic-sqlite-fallback', [
                'driver' => $this->database->driver(),
                'path' => DefaultUserStorage::path($this->config),
            ]);
            PrefabRuntime::recordResolution('users', 'user_provider', 'automatic-sqlite-fallback', [
    ...
    private function resolveAutoLogger(): ?object
    {
        ...
        PrefabRuntime::recordResolution('users', 'logger', 'prefab-capability', ['provider' => $logger['provider']]);
    ...
    private function context(array $context): array
    {
        ...
        if (!array_key_exists('actor_id', $base)) {
            $base['actor_id'] = ($this->actorProvider && method_exists($this->actorProvider, 'id'))
                ? $this->actorProvider->id()
                : null;
        }
